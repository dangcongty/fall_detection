...
class Model(nn.Module):
    ...
    def __init__(self, in_channels, num_class, graph_args,
                 edge_importance_weighting, use_ct = True, **kwargs):
        super().__init__()
...
class ClassifierWithContrastiveHead(nn.Module):
    def __init__(self, in_chan, latent_dim = 1024):
        super().__init__()

        self.latent_layer = nn.Sequential(
            nn.BatchNorm1d(in_chan),
            nn.Mish(inplace=True),
            nn.Linear(in_chan, latent_dim),
        )
